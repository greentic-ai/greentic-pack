...
use packc::event_topics::topics_from_tags;
use serde_json::json;
...
#[test]
fn subscribes_to_survives_into_manifest() {
    let (_tmp, manifest_path) = write_pack("out", "fixture");

    let manifest =
        decode_pack_manifest(&fs::read(&manifest_path).unwrap()).expect("manifest decodes");
    let flow = manifest
        .flows
        .iter()
    ...
    assert!(
        flow.tags.iter().any(|t| t == "event-topic:orders.created"),
        "manifest flow tags missing event topic: {:?}",
        flow.tags
    );
    assert_eq!(topics_from_tags(&flow.tags), vec!["orders.created"]);
}
